  ...
  if (!expanded && !activeThread) {
    return (
      <div
        ...
        className="bg-card rounded-2xl border border-border/30 cursor-pointer hover:bg-white/[0.02] active:bg-white/[0.04] transition-colors"
      >
    ...
    return (
      <div className="bg-card rounded-2xl border border-border/30 overflow-hidden">
        <div
          ...
          className="flex items-center gap-3 p-5 cursor-pointer hover:bg-white/[0.02] transition-colors border-b border-white/[0.04]"
        >
          <MessageCircle size={14} className="text-muted-foreground/40" />
          <span className="text-[12px] font-semibold text-muted-foreground/50 uppercase tracking-wider flex-1">
            Ask the agent
          ...
          {threads.map(thread => {
            ...
            return (
              <div
                ...
                className="flex items-center gap-3 px-5 py-3.5 cursor-pointer hover:bg-white/[0.025] active:bg-white/[0.04] transition-colors"
              >
        ...
        <div className="p-4 border-t border-white/[0.04]">
          <button
            ...
            className="w-full py-2.5 rounded-xl text-[12px] font-semibold text-primary bg-primary/10 hover:bg-primary/15 active:bg-primary/20 transition-colors"
          >
            New conversation
          </button>
        </div>
  ...
  return (
    <div className="bg-card rounded-2xl border border-border/30 overflow-hidden flex flex-col" style={{ maxHeight: 420 }}>
      <div className="flex items-center gap-2 px-4 py-3 border-b border-white/[0.04] shrink-0">
        <button onClick={handleBack} className="p-1 -ml-1 rounded-lg hover:bg-white/[0.06] transition-colors">
          <ArrowLeft size={16} className="text-muted-foreground/60" />
        </button>
        ...
        <button
          ...
          className="p-1 rounded-lg hover:bg-rose-500/10 transition-colors ml-1"
        >
          <Trash2 size={14} className="text-muted-foreground/40" />
        </button>
      </div>
      ...
      {confirmDeleteId === activeThread.id && (
        <div className="px-4 py-3 bg-rose-500/[0.06] border-b border-rose-500/10 shrink-0">
          ...
          <p className="text-[10px] text-muted-foreground/50 mb-3 leading-relaxed">
            Venice messages removed from device. Receipts and onchain proof are not affected.
          </p>
          <div className="flex gap-2">
            <button
              onClick={() => setConfirmDeleteId(null)}
              className="flex-1 py-2 rounded-lg text-[11px] font-semibold text-foreground/70 bg-white/[0.06] hover:bg-white/[0.09] transition-colors"
            >
              Keep it
            </button>
            <button
              onClick={() => handleDeleteConfirm(activeThread.id)}
              className="flex-1 py-2 rounded-lg text-[11px] font-semibold text-rose-400 bg-rose-500/10 hover:bg-rose-500/15 transition-colors"
            >
              Delete
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-0">
              ...
              <div
                className={`px-3 py-2 rounded-xl text-[12px] leading-relaxed ${
                  message.role === 'user'
                    ...
                    : 'bg-white/[0.05] text-foreground/90 rounded-bl-sm'
                }`}
              >
                {message.content}
              </div>
        ...
        {pendingAssistantText && (
          <div className="flex justify-start gap-2">
            ...
            <div className="px-3 py-2 rounded-xl rounded-bl-sm bg-white/[0.05] text-foreground/90 text-[12px] leading-relaxed max-w-[80%]">
              {pendingAssistantText}
              <span className="inline-block w-1.5 h-3 bg-primary/40 animate-pulse ml-0.5 rounded-sm" />
            </div>
          </div>
        )}
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-t border-white/[0.04] shrink-0">
        <input
          ...
          disabled={isStreaming}
          className="flex-1 bg-white/[0.04] border border-white/[0.06] rounded-xl px-3 py-2 text-[12px] text-foreground placeholder:text-muted-foreground/30 outline-none focus:border-primary/30 transition-colors disabled:opacity-50"
        />
        <button
          onClick={() => void handleSend()}
          disabled={!inputValue.trim() || isStreaming}
          className="p-2 rounded-xl bg-primary/10 text-primary hover:bg-primary/20 transition-colors disabled:opacity-30 disabled:pointer-events-none"
        >
          <Send size={14} />
        </button>
      </div>
