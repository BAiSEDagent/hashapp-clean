...
\- \*\*Never hardcode API keys\*\* in source files — use environment variables or \`foundry.toml\` with \`${ENV\_VAR}\` references
...
\- \*\*contract-path\*\*: Must match \`^\[a-zA-Z0-9\_/.-\]+\\.sol:\[a-zA-Z0-9\_\]+$\`. Reject paths with spaces, semicolons, pipes, or backticks.
\- \*\*rpc-url\*\*: Must be a valid HTTPS URL (\`^https://\[^\\s;\|&\]+$\`). Reject non-HTTPS or malformed URLs.
\- \*\*keystore-account\*\*: Must be alphanumeric with hyphens/underscores (\`^\[a-zA-Z0-9\_-\]+$\`).
...
\`\`\`
CDP\_API\_KEY\_ID=your-api-key-id
CDP\_API\_KEY\_SECRET=your-api-key-secret
CDP\_WALLET\_SECRET=your-wallet-secret
\`\`\`
...
\`\`\`bash
export ETHERSCAN\_API\_KEY=your-basescan-api-key
\`\`\`
...
\`\`\`toml
\[etherscan\]
base-sepolia = { key = "${ETHERSCAN\_API\_KEY}", url = "https://api-sepolia.basescan.org/api" }
base = { key = "${ETHERSCAN\_API\_KEY}", url = "https://api.basescan.org/api" }
\`\`\`
...
forge create src/MyContract.sol:MyContract \
 ...
 --account  \
 --verify \
 --etherscan-api-key $ETHERSCAN\_API\_KEY
\`\`\`
