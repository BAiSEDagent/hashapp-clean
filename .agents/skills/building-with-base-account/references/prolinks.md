...
The encoding is optimized per method type (\`wallet\_sendCalls\`, \`wallet\_sign\`, generic JSON-RPC) and uses gzip compression for payloads >= 1KB (50-80% size reduction).
...
\|-----------\|------\|----------\|-------------\|
\| \`method\` \| \`string\` \| Yes \| JSON-RPC method (\`wallet\_sendCalls\`, \`wallet\_sign\`, or any) \|
\| \`params\` \| \`unknown\` \| Yes \| Method parameters \|
\| \`chainId\` \| \`number\` \| No \| Required for generic methods; auto-extracted for \`wallet\_sendCalls\`/\`wallet\_sign\` \|
...
// decoded.method → 'wallet\_sendCalls'
...
if (decoded.method !== 'wallet\_sendCalls') throw new Error('Unexpected method');
