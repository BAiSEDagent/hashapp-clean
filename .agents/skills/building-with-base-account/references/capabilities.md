...
\- \[wallet\_sendCalls\](#wallet\_sendcalls)
\- \[wallet\_getCallsStatus\](#wallet\_getcallsstatus)
...
Capabilities are chain-specific feature flags that describe what a wallet supports. They're discovered via \`wallet\_getCapabilities\` and used in \`wallet\_connect\` and \`wallet\_sendCalls\` calls.
...
const capabilities = await provider.request({
 method: 'wallet\_getCapabilities',
 params: \[userAddress\],
});
...
Check the status of a batch submitted via \`wallet\_sendCalls\`.

\`\`\`typescript
const result = await provider.request({
 method: 'wallet\_getCallsStatus',
 params: \[batchId\],
});
\`\`\`
...
\- \`pm\_getPaymasterStubData\` — for gas estimation
\- \`pm\_getPaymasterData\` — for actual UserOp paymaster data
...
Set \`atomicRequired: true\` in \`wallet\_sendCalls\` to enforce atomic execution. If the wallet doesn't support it, the call fails with error \`5700\`.
...
Collects user profile information (email, phone, address) during transaction flows. Same mechanism as \`payerInfo\` in \`pay()\` but for \`wallet\_sendCalls\`.
...
const builderCodeSuffix = Attribution.toDataSuffix({
 codes: \['bc\_foobar'\], // Register at base.dev
});
