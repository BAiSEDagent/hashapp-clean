...
\- \[wallet\_addSubAccount RPC\](#wallet\_addsubaccount-rpc)
\- \[wallet\_getSubAccounts RPC\](#wallet\_getsubaccounts-rpc)
...
\- Ownership may change across devices/browsers — always call \`wallet\_addSubAccount\` each session
...
\### Via RPC (wallet\_addSubAccount)

\`\`\`typescript
const result = await provider.request({
 method: 'wallet\_addSubAccount',
 params: \[{\
 account: {\
 type: 'create',\
 keys: \[{\
...
\*\*Call \`wallet\_addSubAccount\` at the start of each session\*\* before using the sub account. This is necessary because:
