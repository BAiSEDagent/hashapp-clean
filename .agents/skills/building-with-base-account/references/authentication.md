...
2\. Call \`wallet\_connect\` with the \`signInWithEthereum\` capability
...
Not every wallet supports \`wallet\_connect\`. Fall back to \`eth\_requestAccounts\` + \`personal\_sign\`:
...
try {
 ...
 if (err.code === 4100) {
 const \[address\] = await provider.request({ method: 'eth\_requestAccounts' });
